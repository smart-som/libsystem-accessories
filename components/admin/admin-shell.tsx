...
export function AdminShell({
  ...
  >(null);
  const [readNotificationOrderIds, setReadNotificationOrderIds] = useState<
    string[]
  >([]);
  ...
  const unreadRecentOrders = useMemo(
    () =>
      recentOrders.filter(
        (order) => !readNotificationOrderIds.includes(order.id),
      ),
  ...
  const toggleUtilityPanel = (panel: 'notifications' | 'help' | 'settings') => {
    ...
    if (nextPanel === 'notifications' && recentOrders.length) {
      setReadNotificationOrderIds(
        markAdminNotificationsRead(recentOrders.map((order) => order.id)),
      );
    }
  };
      ...
      <div className="flex min-h-screen gap-0">
        <aside className="sticky top-0 flex h-screen w-full max-w-[250px] shrink-0 flex-col overflow-y-auto border-r border-[color:var(--admin-border)] bg-[var(--admin-sidebar)] px-4 py-5">
          <Link href="/admin" className="flex items-center gap-3 px-1">
            <div className="relative flex h-11 w-11 items-center justify-center overflow-hidden border border-[color:var(--admin-border)] bg-[var(--admin-panel-2)]">
              <Image
                src="/libsystem-logo.jpeg"
                alt="Libsystem Accessories logo"
                fill
                className="object-cover"
              />
            </div>
            <div>
              <p className="font-display text-xl font-semibold text-[var(--admin-text)]">
                Libsystem
              </p>
        ...
        <main className="min-w-0 flex-1 bg-[var(--admin-main)] p-5 text-[var(--admin-text)] lg:p-7">
          <div className="mb-8 flex flex-col gap-4 border-b border-[color:var(--admin-border)] pb-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.24em] text-[var(--admin-subtle)]">
                Admin workspace
              </p>
              <h1 className="mt-2 font-display text-4xl font-semibold text-[var(--admin-text)]">
                {pageTitle}
              </h1>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              ...
              <div className="flex items-center gap-3 border border-[color:var(--admin-border)] bg-[var(--admin-panel)] px-3 py-2">
                ...
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-[var(--admin-subtle)]">
                    {userEmail}
                  </p>
                </div>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5">
                ...
                recentOrders.length ? (
                  <div className="space-y-4">
                    <p className="text-sm text-[var(--admin-muted)]">
                      Orders placed within the last 24 hours show a count badge
                      here, and the latest orders appear first.
                    </p>
                    {recentOrders.map((order) => {
                      const isRecent =
                        ...
                        notificationReferenceTime -
                          ...
                          1000 * 60 * 60 * 24;
                      const isUnread = !readNotificationOrderIds.includes(
                        order.id,
                      );
                      ...
                      return (
                        <div
                          key={order.id}
                          className="border border-[color:var(--admin-border)] bg-[var(--admin-panel)] p-4"
                        >
                    ...
                    <Link
                      href="/admin/orders"
                      onClick={closeUtilityPanel}
                      className="inline-flex text-sm font-semibold text-[var(--admin-text)] underline underline-offset-4"
                    >
              ...
              {activeUtilityPanel === 'help' ? (
                <div className="space-y-4">
                  <p className="text-sm text-[var(--admin-muted)]">
                    Quick guidance for the workflows that are now connected in
                    this admin area.
                  </p>
                  {helpCards.map((card) => (
                    <div
              ...
              {activeUtilityPanel === 'settings' ? (
                <div className="space-y-4">
                  <div className="border border-[color:var(--admin-border)] bg-[var(--admin-panel)] p-4">
                    ...
                    <p className="mt-2 text-sm leading-7 text-[var(--admin-muted)]">
                      Use the sun and moon button in the header to switch
                      between the light and dark admin themes.
                    </p>
                  </div>
                  <div className="border border-[color:var(--admin-border)] bg-[var(--admin-panel)] p-4">
                    <p className="font-semibold text-[var(--admin-text)]">
                    ...
                    <p className="mt-2 text-sm leading-7 text-[var(--admin-muted)]">
                      Sign out from the sidebar whenever you want to return to
                      the customer storefront or switch accounts.
                    </p>
                  </div>
                </div>
              ) : null}
